...
public class SelectDao {
    public JSONArray select(JSONArray ja, Connection con,String id){
        ...
        for(int i=0;i<ja.size();i++) {
            ...
            //计算该课程有多少学生选了
            sql="select count(Snumber) as co form sc where Cnumber="+ map.get("course_id")+"Corder="+map.get("course_seq")+";";
            pstmt=con.prepareStatement(sql);
            rs=pstmt.executeQuery();
            int co=0;
            while(rs.next()){
            ...
            //查询课容量
            sql = "select * from course where Cnumber=" + map.get("course_id") + "Corder=" + map.get("course_seq") + ";";
            pstmt = con.prepareStatement(sql);
            rs=pstmt.executeQuery();
            int capacity=0;
            while (rs.next()){
